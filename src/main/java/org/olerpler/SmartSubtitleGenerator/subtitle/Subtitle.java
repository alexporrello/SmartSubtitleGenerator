...
class Subtitle {
	 ...
	 */
	public Subtitle(String top, String bottom) {
		this.top        = top;
		this.bottom     = bottom;
	 ...
	 */
	public static ArrayList<Subtitle> processSubtitle(String text) {

		ArrayList<Subtitle> toPaint = new ArrayList<Subtitle>();

		Word fullTitle = new Word(text, FONT);

		if(fullTitle.width < SUBTITLE_WIDTH-40) {
			toPaint.add(new Subtitle(fullTitle.word));
		} else {
			...
			for(int i = 0; i < lines.size(); i++) {
				if(i+1 < lines.size()) {
					toPaint.add(new Subtitle(lines.get(i), lines.get(i+=1)));
				} else {
					toPaint.add(new Subtitle(lines.get(i)));
				}
			}
		}
